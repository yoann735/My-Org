...
import { weekPlan, recipeById, recipeTint } from '../data/dataLayer.js';

function SlotCard({ recipeId, leftover, weekend, active, onToggle, onOpen }) {
          ...
          return (
            <div key={d.key} className={'cal-day-block' + (d.weekend ? ' weekend' : '') + (dayOff ? ' day-off' : '')}>
              ...
              <div className="cdb-slot">
                ...
                <SlotCard recipeId={d.midi} leftover={d.midiLeftover} weekend={d.weekend} active={slotActive(d.key, 'midi')} onToggle={toggler(d.key, 'midi')} onOpen={onOpenRecipe} />
              </div>
              <div className="cdb-slot">
                <span className="slot-label"><Icon name="moon" size={11} /> Soir</span>
                <SlotCard recipeId={d.soir} leftover={false} weekend={d.weekend} active={slotActive(d.key, 'soir')} onToggle={toggler(d.key, 'soir')} onOpen={onOpenRecipe} />
              </div>
            </div>
          );
        })}
